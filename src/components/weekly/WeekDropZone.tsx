...
type Props = {
  ...
  onResetTargetDay?: () => void;
};

export const WeekDropZone = forwardRef(
  (
    {
      children,
      onDayChange,
    ...
    useEffect(() => {
      ...
      window.addEventListener("focus", disableBrowserSwipe);
      window.addEventListener("blur", disableBrowserSwipe);

      return () => {
        window.removeEventListener("focus", disableBrowserSwipe);
        window.removeEventListener("blur", disableBrowserSwipe);
      };
    }, [handleEdgeChange, onDayChange, onEdgeChange, onResetTargetDay]);
